...
instance Show Expr where
  ...
  showsPrec p (Ap e1 e2) =
    showParen (p > 2) $
      showsPrec 2 e1 . (' ' :) . showsPrec 3 e2
  showsPrec p (Let bg e) =
    showParen (p > 0) $
      ("let {" ++) . ssBindGroup bg . ("} in " ++) . showsPrec 0 e
  showsPrec p (Case e pes) =
    showParen (p > 0) $
      ("case " ++) . showsPrec 0 e . (" of {" ++) . (join ";\n\t" alts) . ('}' :)
    where
      alts = [showsPrec 0 p . ssRhs " -> " rhs | (p, rhs) <- pes]
  showsPrec p (Lambda (ps, Rhs e)) =
    showParen (p > 0) $
      ("\\" ++) . join " " (map (showsPrec 1) ps) . ("-> " ++) . showsPrec 0 e
  showsPrec p (ESign e sc) =
    showParen True $
