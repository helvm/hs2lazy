...
import Text.ParserCombinators.Parsec hiding (many, satisfy, (<|>))
import Text.ParserCombinators.Parsec.Expr
import Prelude hiding (Ap, Type)
...
classdecl = do
  ...
  where
    test (TyAp (TyCon id) t@(TyVar _)) = return (id, t)
...
instdecl = do
  ...
  where
    pred (TyAp (TyCon id) t) = do t' <- test inst t; return (id, t)
...
toSimpletype (TyAp l (TyVar v)) = do
  (c, vs) <- toSimpletype l
  return (c, vs <> [v])
...
type_ = chainr1 btype ap
  where
    ap = do tokenRArrow; return arrow
...
atype :: HsParser Type
atype =
  do t <- brackets type_; return (TyAp (TyCon "[]") t)
    <|> do
      ts <- parenList type_
      case ts of
        [t] -> return t
...
opTable op =
  [ [ Infix (op (InfixN n)) AssocNone,
      Infix (op (InfixL n)) AssocLeft,
      Infix (op (InfixR n)) AssocRight
    ]
  | n <- reverse [0 .. 9]
  ]
...
conid = token' test
  where
    test (TokenConId id) = Just id
    test _ = Nothing

consym :: HsParser String
consym = token' test
  where
    test (TokenConOp id) = Just id
    test _ = Nothing

consymF :: Fixity -> HsParser String
consymF f = token' test
  where
    test (TokenConOp id) | fixity id == f = Just id
    test _ = Nothing
...
varid = token' test
  where
    test (TokenId id) = Just id
    test _ = Nothing

varsym :: HsParser String
varsym = token' test
  where
    test (TokenOp id) = Just id
    test _ = Nothing

varsymF :: Fixity -> HsParser String
varsymF f = token' test
  where
    test (TokenOp id) | fixity id == f = Just id
    test _ = Nothing
...
litInt = token' test
  where
    test (TokenInt i) = Just i
    test _ = Nothing

litChar :: HsParser String
litChar = token' test
  where
    test (TokenChar i) = Just i
    test _ = Nothing

litStr :: HsParser String
litStr = token' test
  where
    test (TokenStr i) = Just i
    test _ = Nothing
