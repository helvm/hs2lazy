...
makeSel con i e = expr
  where
    ...
    expr =
      ap
        e
        [ if i == conTag con then receiver else eError
...
matchConLastClause us def grp = do
  (_, e) <- matchConClause us def grp
  return e
...
groupCon qs@((PCon c _ : _, _) : _) = (c, qs') : groupCon qs''
  where
    (qs', qs'') = partition (\(PCon c' _ : _, _) -> c == c') qs

isCovered :: [(Const, [Equation])] -> Bool
isCovered grps = n == 0 || length grps == n
  where
    n = tyconNumCon $ conTycon $ fst $ head grps
