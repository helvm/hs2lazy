module HS2Lazy.Static where

import Data.List (partition, union, (\\))
import qualified Data.List (find)
import qualified HS2Lazy.Parser as P
import HS2Lazy.PatComp (patBindings)
...
splitDecls tds =
  ...
  where
    dataDecls = [dd | P.DataDecl dd <- tds]
...
anVarDecl dt (i, ps, rhs) = (i, alt)
  where
    alt = (map (anPat dt) ps, anRhs dt rhs)
...
anPatBind dt n (p, rhs) = conf : bs
  where
    conf = (newvar, [([], anRhs dt $ makeBindRhs p rhs)])
...
makeBindRhs p (P.Rhs e ds) = P.Rhs e' ds
  where
    e' = P.Case e [(P.AsPat "@u" p, P.Rhs (P.Var "@u") [])]
makeBindRhs p (P.Guarded gds ds) = P.Guarded gds' ds
  where
    gds' =
      [ (cond, P.Case e [(P.AsPat "@u" p, P.Rhs (P.Var "@u") [])])
      | (cond, e) <- gds
      ]
...
anSigDecl (tce, _) (is, ctx, t) = [(i, sc) | i <- is]
  where
    sc = quantifyAll (anContext ctx :=> anType tce t)
...
anDataLhs ctx con vs n arities =
  ...
  where
    k = foldr Kfun Star (replicate (length vs') Star)
...
selectors (sup, id, v, decls) as = supers ++ methods
  where
    supers =
      [ superSelector s id k dictsize
      | ((s, _), k) <- zip sup [0 ..]
      ]
    methods =
      [ tupleSelector m k dictsize
...
anDefaultMethodDecl dt as decls = map makeExpl impls
  where
    impls = anVarDecls dt [a | P.VarDecl a <- decls]
    makeExpl (id, alts) =
...
anInstDecl dt@(tce, _) (ctx, id, t, decls) ce =
  ...
  where
    addinst = T.addInst ps p (Var dictid)
    ps = anContext ctx
    t' = anType tce t
    p = IsIn id t'
    dataid = makeDataId t'
