...
preludeSynonyms =
  [ Synonym "String" Star [] (list tChar)
  ]
...
eTrue = Con con
  where
    Just con = find (\c -> conName c == "True") preludeConstrs

eFalse = Con con
  where
    Just con = find (\c -> conName c == "False") preludeConstrs

eCons = Con con
  where
    Just con = find (\c -> conName c == ":") preludeConstrs
...
instance HasVar Expr where
  ...
  freeVars (Let bg e) =
    fvBindGroup bg
      `union` (freeVars e \\ map fst (bindings bg))
