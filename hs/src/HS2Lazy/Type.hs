...
lookupEnv (Env as ras) i =
  case lookup i ras of
    ...
    Nothing -> find as
      where
        find [] = fail ("unbound identifier: " ++ i)
...
tiExpr ce env e@(Var i) =
  do
    sc_or_t <- lookupEnv env i
    case sc_or_t of
      Left sc -> do
        (ps :=> t) <- freshInst sc
        return (ps, t, foldl Ap e (map ClassPH ps))
      Right t -> return ([], t, RecPH i)
tiExpr ce env e@(Con con) = do
  (ps :=> t) <- freshInst (conScheme con)
  return (ps, t, e)
tiExpr ce env e@(Lit l) = do
  (ps, t) <- tiLit l
  return (ps, t, e)
tiExpr ce env (Ap e f) = do
  (ps, te, e') <- tiExpr ce env e
  (qs, tf, f') <- tiExpr ce env f
  t <- newTVar Star
  unify (tf `fn` t) te
  return (ps ++ qs, t, Ap e' f')
tiExpr ce env (Let bg e) = do
  (ps, as, bg') <- tiBindGroup ce env bg
  (qs, t, e') <- tiExpr ce (extend env as) e
  return (ps ++ qs, t, Let bg' e')
tiExpr ce env (Case e pses) = do
  (ps, te, e') <- tiExpr ce env e
  tf <- newTVar Star
  t <- newTVar Star
  unify (te `fn` t) tf
  ...
  where
    alts = [([p], e) | (p, e) <- pses]
tiExpr ce env (Lambda alt) = do
  (ps, t, alt') <- tiAlt ce env alt
...
tiAlts ce env alts t = do
  r <- mapM (tiAlt ce env) alts
...
tiGuard ce env (cond, e) =
  do
    (ps, tcond, cond') <- tiExpr ce env cond
    unify tcond tBool
...
resolve ce s recs ps alts = map resolveAlt alts
  where
    dictVars = [c ++ '#' : v | IsIn c (TVar (Tyvar v _)) <- ps]
    env =
      ResolveEnv
        { reParam = zip ps (map Var dictVars),
          reRec =
            [ (i, foldl Ap (Var i) (map ClassPH ps))
            | (i, ps) <- recs
            ],
          reSubst = s,
          reClass = ce
...
resolveSuper re pes@(_ : _) p =
  ...
  where
    pes' =
      [ (IsIn sup v, Var (cls ++ ">>" ++ sup) `Ap` e)
      | (IsIn cls v, e) <- pes,
        sup <- super (reClass re) cls
      ]

resolveBindGroup re (es, iss) = (es', iss')
  where
    es' = [(i, sc, resolveAlts alts) | (i, sc, alts) <- es]
...
tiExpl ce env (i, sc, alts) =
  do
    ...
    if sc /= sc'
      then
        fail
          ( "signature too general: expected"
              ++ show sc
              ++ ", but inferred "
              ++ show sc'
          )
...
tiSeq ti ce env (bs : bss) = do
  (ps, as, bs') <- ti ce env bs
...
tiProgram ce as bgs = runTI $
  do
    (ps, as', bgs') <- tiSeq tiBindGroup ce (makeEnv as) bgs
    s <- getSubst
